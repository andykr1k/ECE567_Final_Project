...
from craftax.craftax.envs.craftax_symbolic_env import get_flat_map_obs_shape
...
def make_train(config):
    ...
    assert config["ENV_NAME"] == "Craftax-Symbolic-v1", "For inv only training, use Craftax-Symbolic-v1"
    env = make_craftax_env_from_name(
        config["ENV_NAME"], not config["USE_OPTIMISTIC_RESETS"]
    )
    env_params = env.default_params

    inv_only = config["ICM_INV_ONLY"]
    if inv_only:
        icm_view_obs_start = get_flat_map_obs_shape()
    else:
        icm_view_obs_start = 0
    ...
    def train(rng):
        ...
        if config["TRAIN_ICM"]:
            ...
            # --- MODIFICATION ---
            # Set the shape to only the length of the non-visual slice
            icm_obs_shape = obs_shape - icm_view_obs_start 
            # --------------------
            ...
            # --- MODIFICATION ---
            if inv_only:
                # Init with the smaller shape
                icm_encoder_network_params = icm_encoder_network.init(
                    _rng, jnp.zeros((1, icm_obs_shape)) 
                )
            else:
                icm_encoder_network_params = icm_encoder_network.init(
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                if config["TRAIN_ICM"]:
                    # --- MODIFICATION ---
                    if inv_only:
                        icm_last_obs = last_obs[:, icm_view_obs_start:]
                        icm_obsv = obsv[:, icm_view_obs_start:]
                        
                        latent_obs = ex_state["icm_encoder"].apply_fn(
                            ex_state["icm_encoder"].params, icm_last_obs
                        )
                        latent_next_obs = ex_state["icm_encoder"].apply_fn(
                            ex_state["icm_encoder"].params, icm_obsv
                        )
                    else:
                        latent_obs = ex_state["icm_encoder"].apply_fn(
            ...
            def _update_ex_epoch(update_state, unused):
                def _update_ex_minbatch(ex_state, traj_batch):
                    def _inverse_loss_fn(
                        ...
                        # --- MODIFICATION ---
                        if inv_only:
                            icm_batch_obs = traj_batch.obs[:, icm_view_obs_start:]
                            icm_batch_next_obs = traj_batch.next_obs[:, icm_view_obs_start:]

                            latent_obs = ex_state["icm_encoder"].apply_fn(
                                icm_encoder_params, icm_batch_obs
                            )
                            latent_next_obs = ex_state["icm_encoder"].apply_fn(
                                icm_encoder_params, icm_batch_next_obs
                            )
                        else:
                            latent_obs = ex_state["icm_encoder"].apply_fn(
                    ...
                    def _forward_loss_fn(icm_forward_params, traj_batch):
                        ...
                        if inv_only:
                            ...
                            latent_obs = ex_state["icm_encoder"].apply_fn(
                                ex_state["icm_encoder"].params, icm_batch_obs
                            )
                            latent_next_obs = ex_state["icm_encoder"].apply_fn(
                                ex_state["icm_encoder"].params, icm_batch_next_obs
                            )
                        else:
                            latent_obs = ex_state["icm_encoder"].apply_fn(
...
def run_ppo(config):
    ...
    if config["USE_WANDB"]:
        wandb.init(
            ...
            + "M"
            + ("_inv_only" if config["ICM_INV_ONLY"] else ""),
        )
...
if __name__ == "__main__":
    ...
    # Custom
    parser.add_argument("--icm_inv_only", action="store_true")
